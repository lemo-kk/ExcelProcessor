...
namespace ExcelProcessor.Data.Infrastructure
{
	public sealed class OracleDialect : ISqlDialect
	{
		...
		public string BuildCreateTable(string tableName, IDictionary<string, string> columns)
		{
			...
			return $"CREATE TABLE {QuoteIdentifier(tableName)} (Id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, {cols})";
		}
		public string BuildInsert(string tableName, IReadOnlyList<string> columnNames)
		{
			...
			var pars = string.Join(", ", columnNames.Select(name => $":{name}"));
		...
		public string GetExistsTableSql(string tableName) => "SELECT COUNT(*) FROM user_tables WHERE table_name = UPPER(:TableName)";
		public string MapType(string neutralType)
		{
			switch (neutralType.ToUpper())
			{
				...
				case "DECIMAL(10,2)":
				case "DECIMAL(15,2)": return "NUMBER(15,2)";
				case "DATE": return "DATE";
				case "DATETIME": return "TIMESTAMP";
				case "TEXT": return "CLOB";
				case "VARCHAR(50)":
				case "VARCHAR(100)":
				case "VARCHAR(200)": return "VARCHAR2" + neutralType.Substring("VARCHAR".Length);
				default: return "CLOB";
			}
		}
	}
